...
abstract class Logging() {
  ...
  private def initializeLogging(isInterpreter: Boolean, silent: Boolean): Unit = {
    ...
    if (Logging.isLog4j12) {
      ...
      val log4j12Initialized: Boolean = LogManager.getRootLogger.getAllAppenders.hasMoreElements
      // scalastyle:off println
      if (!log4j12Initialized) {
        Logging.defaultSparkLog4jConfig = true
        val defaultLogProps = "org/apache/spark/log4j-defaults.properties"
        Option(Utils.getMainClassLoader.getResource(defaultLogProps)) match {
          case Some(url) =>
      ...
      val rootLogger: log4j.Logger = LogManager.getRootLogger
      ...
      if (isInterpreter) {
        ...
        val replLogger: log4j.Logger = LogManager.getLogger(logName)
        val replLevel: Level = Option(replLogger.getLevel).getOrElse(Level.WARN)
...
private[schnapps] object Logging {
  ...
  try {
    ...
    val bridgeClass: Class[_] = Utils.classForName("org.slf4j.bridge.SLF4JBridgeHandler")
  ...
  private def isLog4j12: Boolean = {
    ...
    val binderClass: String = StaticLoggerBinder.getSingleton.getLoggerFactoryClassStr
